...
const setUserInfo = (targetUserInfo) => {
    ...
    const phoneNumber = targetUserInfo.PHONE_NUMBER ? targetUserInfo.PHONE_NUMBER : "입력 없음";
    const address = targetUserInfo.ZIP_CODE
        ? targetUserInfo.ZIP_CODE + targetUserInfo.ADDRESS1 + targetUserInfo.ADDRESS2
        : "입력 없음";
...
$moreInformationForm.addEventListener('click', function (e) {
    ...
    if (e.target.classList.contains('edit-btn')) {
        ...
        const containerEl = targetEl.parentElement.parentElement.querySelector('.button-container');
    ...
    if (e.target.classList.contains('cancel-button')) {
        const targetEl = e.target;
        const inputEl = targetEl.parentElement.parentElement.querySelector('.input-field > input');
        inputEl.style.display = 'none';
        const labelEl = targetEl.parentElement.parentElement.querySelector('.input-field > label');
        labelEl.style.display = 'block';
        const containerEl = targetEl.parentElement;
        containerEl.style.display = 'none';
        const editBtnEl = targetEl.parentElement.parentElement.querySelector('.input-field > .edit-btn');
...
$moreInformationForm.addEventListener('submit', async function (e) {
    ...
    const params = {
        FULL_NAME: e.target.FULL_NAME.value.trim() === '' ? null : e.target.FULL_NAME.value.trim(),
        PHONE_NUMBER: e.target.PHONE_NUMBER.value.trim() === '' ? null : e.target.PHONE_NUMBER.value.trim(),
    };
